...
int PM_parseFrom(ParameterManager *p, FILE *fp, char comment)
{
    ...
    //    printf("%s",line);
        ...
        else
        {
            ...
            strcpy(singleLine,stripLine(line,comment));
            RemoveSpaces(singleLine);
            printf("Single: %s\n",line);
        ...
        if(check1==1 && check2==1)
        {
            ...
            rVal=1;
            singleOrNot=1;
            tokenize(singleLine,p);
        }
            //If single line is not valid, read uo to 100 lines, concatinate them, and check that concatinated
            //line for both ; and =
        else
        {
            strcpy(concat,singleLine);
            for(int t=0;t<100;t++)
            {
                check1=0;
                check2=0;
                ...
                {
                    strcpy(nextLine,stripLine(line2,comment));
                        ...
                        //    Fix for pName on same line
                            if(isalpha(concat[q+1]))
                            {
                                    ...
                                    else
                                    {
                                        ...
                                        strcpy(extra,stripLine(extraCat,comment));
                                        strcpy(concat,extraCat);
                                        RemoveSpaces(concat);
